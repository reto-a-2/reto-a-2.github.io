(() => {
    ...
    card.addEventListener('click', (event) => {
      ...
      if (dialogAmazon) {
        ...
        if (amazonUrl) {
          ...
          dialogAmazon.rel = 'noopener noreferrer';
        } else {
          dialogAmazon.removeAttribute('target');
          dialogAmazon.removeAttribute('rel');
        }
      }
  ...
  const orderCopyInstructions = document.querySelector('[data-copy-instructions]');
  ...
  orderItems.forEach((item) => {
    ...
    const catalogCard = [...bookCards].find((card) => card.dataset.title === item.dataset.title);
    const available = catalogCard?.dataset.availability === 'available';
    item.dataset.orderAvailability = available ? 'available' : 'upcoming';
    item.classList.toggle('is-unavailable', !available);
    item.setAttribute('aria-disabled', String(!available));
    input.disabled = !available;
    if (minus) minus.disabled = !available;
    if (plus) plus.disabled = !available;

    if (!available) {
      input.value = '0';
      const status = document.createElement('span');
      status.className = 'professional-book-status';
      status.textContent = 'Próximamente';
      item.querySelector('.quantity-stepper')?.before(status);
    }
  ...
  orderOpen?.addEventListener('click', () => {
    ...
    if (orderCopyInstructions) orderCopyInstructions.hidden = true;
  ...
  const buildProfessionalRequest = () => {
    const order = updateProfessionalOrder();
    if (!order.ready) {
      showToast('El pedido profesional debe sumar al menos 10 ejemplares.');
      return null;
    }
  ...
  orderCopy?.addEventListener('click', async () => {
    ...
    const fullRequest = [
      'PARA',
      'noramontalbaretoados@gmail.com',
      '',
      'ASUNTO',
      request.subject,
      '',
    ...
    try {
      ...
      if (orderCopyInstructions) {
        orderCopyInstructions.hidden = false;
        orderCopyInstructions.focus({ preventScroll: true });
      }
      showToast('Solicitud copiada. Seguid las instrucciones mostradas bajo los botones.');
    } catch {
